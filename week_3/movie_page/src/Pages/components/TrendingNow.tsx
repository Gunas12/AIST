...
function TrendingNow() {
  ...
  const fetchData = () => {
    axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=2872128c1cdccd2dce197bbadac76051&page=1`)
      .then((response) => {
      ...
      <div className='genres-container img_cart'>
        {movies.slice((currentPage - 1) * cardsPerPage, currentPage * cardsPerPage).map((item) => (
          <div key={item.id} className="krt">
           ...
           <div className="divt">
            <p> {formatPopularity(item.popularity)} </p>
            <p><FaEye className=".ico"  style={{width: '26px', height: '26px',margin:"5px"}}/></p>
            </div>
          </div>
        ))}
      </div>
